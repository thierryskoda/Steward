...
create_stub dirname "scripts"

if output=$(env PATH="$STUB_BIN:/bin" /bin/bash scripts/doctor.sh 2>&1); then
...
create_stub agent
if output=$(env PATH="$STUB_BIN:/bin" /bin/bash scripts/doctor.sh 2>&1); then
  echo "Expected doctor to fail without Git on PATH." >&2
  echo "$output" >&2
  exit 1
fi
if [[ "$output" != *"FAIL: Git present (install Git and ensure git is on PATH)"* ]]; then
  echo "Doctor did not explain how to fix the missing Git CLI." >&2
  echo "$output" >&2
  exit 1
fi
create_stub git "git version 2.50.0"

for supported_cli in agent codex claude; do
  rm -f "$STUB_BIN/agent" "$STUB_BIN/codex" "$STUB_BIN/claude"
  create_stub "$supported_cli"
  output=$(env PATH="$STUB_BIN:/bin" /bin/bash scripts/doctor.sh)
  ...
  if [[ "$output" != *"OK: Git present (git version 2.50.0)"* ]]; then
    echo "Doctor did not report the detected Git CLI." >&2
    echo "$output" >&2
    exit 1
  fi
done

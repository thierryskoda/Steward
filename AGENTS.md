...
- **Module ownership (apps/runtime/src):**
  ...
  - **features:** domain behavior—config (DB-backed project config), categories/findings (SQLite-backed finding store/service, detection/generation agents, stale-context scan/revalidation, implementation worker/processor, option hints), rules (SQLite-backed proposals, transcript subscriber, rule capture, rule apply worker, rejected-finding learning processor), continual-learning (DB-backed index, timer runner), documentation-refresh (bounded read-only Codex review plus SQLite run ledger), next-commitment (manual, bounded, same-project recommendation plus SQLite run ledger), context (DB-backed generated snapshots, repo-scope config, rules/project-context builders), workflow (allowed transitions and workflow log events).
...
- **Codex task handoff (`apps/menubar/electron`):** the Electron host owns the local app-server client and uses a dedicated project-scoped connection plus a fresh permission-profile id for each submission. Immediately before handoff, the runtime compares the recommendation's captured Git snapshot with the selected project and supersedes stale work. A current recommendation can create, name, run, and open one persisted Desktop-visible review task only after a `final_answer` agent message exists; renderer input is limited to the recommendation run id and never supplies a path, prompt, or URL. The handoff requires the exact selected project as the sole runtime workspace root and the signed-in Desktop ChatGPT account. It marks the selected root untrusted for executable Codex configuration, denies root filesystem access, disables automatic project-doc injection, blocks effective MCP servers and network access, disables host notifications and telemetry, and suppresses skills, subagents, web search, progress commentary, local environments, and both Codex shell implementations. The task reviews only the bounded evidence packet already produced and provenance-checked by the project-isolated recommendation agent; it does not independently reopen repository files or task history.
...
- Documentation refresh consumes the shared stabilized Git change batch, uses a cross-project Codex lease, and publishes report-only findings; it never mutates target repositories.
- Next-commitment review is manual, project-isolated, read-only, and advisory. Codex has no shell or general filesystem access in this flow; a Steward-owned allowlisted MCP broker serves bounded immutable project/task evidence and records successful content reads. The reported file, repository-state, and task inspection lists must exactly match that audit and the captured scope before persistence. Recent-task listing is selection metadata rather than task-content inspection. The flow may persist one evidence-backed recommendation, an explicit no-recommendation result, or a blocker; it never starts implementation automatically.
- The menubar starts every known project runtime after app readiness; selection controls presentation, not runtime liveness.
